...
{
    public sealed class OUTPUT_COMPENSATION : NativeType
    {
        private OUTPUT_COMPENSATION()
        {
            Name = "OUTPUT_COMPENSATION";
            {
        ...
        public static readonly OUTPUT_COMPENSATION Inst = new OUTPUT_COMPENSATION();
        public override IField Create(JToken token)
        {
            var res = new OUTPUT_COMPENSATIONField(token);
            FixUp(token, res);
            return res;
        }
    ...
    public sealed class OUTPUT_COMPENSATIONField : PreDefinedField
    {
        public OUTPUT_COMPENSATIONField(JToken token)
        {
            Debug.Assert(token == null || token is JArray);
            var array = token as JArray;
            fields.Add(Tuple.Create(new RealField(array?[0]) as IField, 0));
