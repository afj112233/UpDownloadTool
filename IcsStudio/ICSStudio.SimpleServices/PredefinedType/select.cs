...
{
    public sealed class SELECT : NativeType
    {
        private SELECT()
        {
            Name = "SELECT";
        ...
        public static readonly SELECT Inst = new SELECT();
        public override IField Create(JToken token)
        {
            var res = new SELECTField(token);
            FixUp(token, res);
            return res;
        }
    ...
    public sealed class SELECTField : PreDefinedField
    {
        public SELECTField(JToken token)
        {
            ...
            fields.Add(Tuple.Create(new Int8Field(array?[6]) as IField, 20));
        }
    }
}