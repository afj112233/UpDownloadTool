...
{
    public sealed class LEAD_LAG : NativeType
    {
        private LEAD_LAG()
        {
            Name = "LEAD_LAG";
        ...
        public static readonly LEAD_LAG Inst = new LEAD_LAG();
        public override IField Create(JToken token)
        {
            var res = new LEAD_LAGField(token);
            FixUp(token, res);
            return res;
        }
        ...
        public override int BitSize => 640;
        public override int ByteSize => 80;
        public const int size = 80;
    }
    public sealed class LEAD_LAGField : PreDefinedField
    {
        public LEAD_LAGField(JToken token)
        {
            ...
            fields.Add(Tuple.Create(new Int8Field(array?[11]) as IField, 44));
            fields.Add(Tuple.Create(new RealField(array?[12]) as IField, 48));
            fields.Add(Tuple.Create(new RealField(array?[13]) as IField, 52));
            fields.Add(Tuple.Create(new Int32Field(array?[14]) as IField, 56));
