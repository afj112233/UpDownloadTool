...
{
    public sealed class DCI_MONITOR : NativeType
    {
        private DCI_MONITOR()
        {
            Name = "DCI_MONITOR";
        ...
        public static readonly DCI_MONITOR Inst = new DCI_MONITOR();
        public override IField Create(JToken token)
        {
            var res = new DCI_MONITORField(token);
            FixUp(token, res);
            return res;
        }
    ...
    public sealed class DCI_MONITORField : PreDefinedField
    {
        public DCI_MONITORField(JToken token)
        {
            ...
            fields.Add(Tuple.Create(new Int8Field(array?[8]) as IField, 20));
            fields.Add(Tuple.Create(new Int8Field(array?[9]) as IField, 21));
            fields.Add(Tuple.Create(new Int8Field(array?[10]) as IField, 22));
            fields.Add(Tuple.Create(new Int8Field(array?[11]) as IField, 23));
            fields.Add(Tuple.Create(new Int32Field(array?[12]) as IField, 24));
            fields.Add(Tuple.Create(new Int32Field(array?[13]) as IField, 28));
        }
    }
}