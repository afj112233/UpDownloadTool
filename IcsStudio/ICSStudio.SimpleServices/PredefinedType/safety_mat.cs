...
{
    public sealed class SAFETY_MAT : NativeType
    {
        private SAFETY_MAT()
        {
            Name = "SAFETY_MAT";
            ...
            }
            {
                var member = new DataTypeMember
                {
                    Name = "ShortCircuitDetectDelayTime",
                    DisplayStyle = DisplayStyle.Decimal,
                    ExternalAccess = ExternalAccess.ReadWrite,
                    ByteOffset = 8,
                    BitOffset = 0,
        ...
        public static readonly SAFETY_MAT Inst = new SAFETY_MAT();
        public override IField Create(JToken token)
        {
            var res = new SAFETY_MATField(token);
            FixUp(token, res);
            return res;
        }
    ...
    public sealed class SAFETY_MATField : PreDefinedField
    {
        public SAFETY_MATField(JToken token)
        {
            ...
            fields.Add(Tuple.Create(new Int32Field(array?[6]) as IField, 8));
            fields.Add(Tuple.Create(new Int8Field(array?[7]) as IField, 12));
            fields.Add(Tuple.Create(new Int8Field(array?[8]) as IField, 13));
            fields.Add(Tuple.Create(new Int8Field(array?[9]) as IField, 14));
            fields.Add(Tuple.Create(new Int8Field(array?[10]) as IField, 15));
            fields.Add(Tuple.Create(new Int8Field(array?[11]) as IField, 16));
