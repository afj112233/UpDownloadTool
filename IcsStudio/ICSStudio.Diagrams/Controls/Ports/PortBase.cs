...
namespace ICSStudio.Diagrams.Controls
{
	public abstract class PortBase : Control, IPort
	{
		...
		public virtual void UpdatePosition()
		{
			...
			if (canvas != null)
				Center = this.TransformToAncestor(canvas).Transform(new Point(this.ActualWidth / 2, this.ActualHeight / 2))+new Vector(1,1);
			else
		...
		protected override void  OnPreviewMouseLeftButtonDown(System.Windows.Input.MouseButtonEventArgs e)
		{
			if (CanCreateLink)
            {
                var port = this as AttachmentPort;
                if (port !=null&&port.IsAttach)
                {
                    //port.IsAttach = false;
                    port.DelLink();
                    e.Handled = true;
				...
				if (view != null)
				{
                    if (view.Items.OfType<ILink>().Where(l => Equals(l.Source, this) || Equals(l.Target, this)).ToList().Count > 0)
                    {
                        e.Handled = true;
                        return;
                    }
