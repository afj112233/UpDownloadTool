...
namespace ICSStudio.Diagrams
{
	public class DiagramView : Canvas
	{
		...
		public DiagramView()
		{
            ...
            this.MouseWheel += DiagramView_MouseWheel;
		}

        public bool CanDrag { set; get; } = true;

        private void DiagramView_MouseWheel(object sender, MouseWheelEventArgs e)
        {
        ...
        void DiagramView_LayoutUpdated(object sender, EventArgs e)
		{
            ...
            {
                n.UpdatePosition();
                if (n.ModelElement is BranchFlowNode)
                {
                    (n.GetContent() as Branch)?.Update(this);
                }
            }
		}
        ...
        {
            var branches = Children.OfType<Node>().Where(n=>n.ModelElement is BranchFlowNode);
            foreach (var branch in branches)
            {
                (branch.GetContent() as Branch).Update(this);
            }
        }
