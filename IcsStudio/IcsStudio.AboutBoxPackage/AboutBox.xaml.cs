...
namespace ICSStudio.AboutBoxPackage
{
    ...
    public partial class AboutBox
    {
        ...
        {
            InitializeComponent();
            versionLabel.Content = "English".Equals(LanguageInfo.CurrentLanguage) ? "Version - " : "版本 - ";
            reserved.Content = "English".Equals(LanguageInfo.CurrentLanguage) ? "All Rights Reserved." : "保留所有权利";
            info.Content = "English".Equals(LanguageInfo.CurrentLanguage) ? "Product details:" : "产品详情：";
            okButton.Content = "English".Equals(LanguageInfo.CurrentLanguage) ? "OK" : "确定";
        }
        ...
        public string Version
        {
            get
            {
                // use ICSStudio.UIServicesPackage.dll version
                var versionService = Package.GetGlobalService(typeof(SVersionService)) as IVersionService;
                string result = versionService?.GetCoreVersion();
        ...
        private string CalculatePropertyValue<T>(string propertyName, string xpathQuery)
        {
            ...
            if (attributes.Length > 0)
            {
                ...
                PropertyInfo property = attrib.GetType()
