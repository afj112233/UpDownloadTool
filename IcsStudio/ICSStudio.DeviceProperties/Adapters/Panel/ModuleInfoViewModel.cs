...
namespace ICSStudio.DeviceProperties.Adapters.Panel
{
    ...
    public class ModuleInfoViewModel : DeviceOptionPanel
    {
        ...
        public ModuleInfoViewModel(UserControl panel, ModifiedDIOEnetAdapter modifiedAdapter)
            : base(panel)
        {
        ...
        public string ModuleIdentity
        {
            get
            {
                if (_descriptor != null)
                {
                    int vendor = ModifiedAdapter.OriginalAdapter.Vendor;
                    int productCode = ModifiedAdapter.OriginalAdapter.ProductCode;
                    int productType = ModifiedAdapter.OriginalAdapter.ProductType;
                    int majorRevision = ModifiedAdapter.OriginalAdapter.Major;
            ...
            if (result == MessageBoxResult.Yes)
            {
                ThreadHelper.JoinableTaskFactory.RunAsync(async delegate
                {
                    ...
                    ResettingDialog dialog = new ResettingDialog
                    {
                        ...
                        DataContext = new DeviceModuleResetting(ModifiedAdapter.OriginalAdapter)
                    };
        ...
        private void ExecuteRefreshCommand()
        {
            ThreadHelper.JoinableTaskFactory.RunAsync(async delegate
            {
                ...
                var viewModel = new DeviceModuleRefreshing(ModifiedAdapter.OriginalAdapter);
