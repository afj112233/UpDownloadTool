...
namespace OxyPlot
{
    ...
    public partial class PlotModel : Model, IPlotModel
    {
        ...
        /// </summary>
        public PlotModel(ITrend trend =null,object syncRoot=null)
        {
        ...
        public event EventHandler CompleteArrange;

        public void DoCompleteArrange()
        {
            CompleteArrange?.Invoke(this,new EventArgs());
        }
        /// <summary>
        ...
        public Axis DefaultXAxis
        {
            get
            {
                if (Trend != null)
                {
                    var axis = Axes.FirstOrDefault(a => a.Position == AxisPosition.Bottom && a.IsAxisVisible);
                    if (axis != null) return axis;
                }
                return _defaultXAxis;
            }
            private set { _defaultXAxis = value; }
        }
        ...
        public Axis DefaultYAxis
        {
            get
            {
                if (Trend != null)
                {
                    var axis = Axes.FirstOrDefault(a => a.Position == AxisPosition.Left && a.IsAxisVisible);
                    if (axis != null) return axis;
                }
                return _defaultYAxis;
            }
            private set { _defaultYAxis = value; }
        }
        ...
        void IPlotModel.AttachPlotView(IPlotView plotView)
        {
            ...
            //AttachPlotViewHandle?.Invoke(this,new EventArgs());
        }

        public void JustUpdateSeriesData()
        {
            lock (this.SyncRoot)
            {
                SeriesGetData();
        ...
        //public event EventHandler AttachPlotViewHandle;
        ...
        public Series.Series GetSeriesFromPoint(ScreenPoint point, double limit = 100)
        {
            ...
            var list = (InIsolated && ScaleOption == ScaleOption.Same)
                ? Series.Reverse()
        ...
        public bool NeedSeriesGetData { set; get; } = true;
        /// <summary>
        ...
        void IPlotModel.Update(bool updateData)
        {
            lock (this.SyncRoot)
            {
                try
                {
                    ...
                    this.OnUpdating();
                    if (updateData && !IsLoad && !IsZoom&& NeedSeriesGetData)
                        this.SeriesGetData();
                    // Updates the default axes
                    this.EnsureDefaultAxes();

                    //离散trend同步设置
                    var visibleSeries = InIsolated&&ScaleOption==ScaleOption.Same?Series.ToArray(): this.Series.Where(s => s.IsVisible).ToArray();
        ...
        private void EnsureDefaultAxes()
        {
            ...
            var areAxesRequired =(InIsolated&&ScaleOption==ScaleOption.Same)? this.Series.Any(s => s.AreAxesRequired()): this.Series.Any(s => s.IsVisible && s.AreAxesRequired());
            ...
            {
                if (InIsolated&&ScaleOption==ScaleOption.Same)
                {
        ...
        private void UpdateMaxMin(bool isDataUpdated)
        {
            ...
            if (isDataUpdated)
            {
                ...
                {
                    if (ScaleOption == ScaleOption.UsingPen)
                    {
                        foreach (var s in this.Series)
                        {
                            s.UpdateMaxMin();
            ...
            else
            {
                if (ScaleOption == ScaleOption.UsingPen)
                {
                    var scaleSeries = Series.FirstOrDefault(s => ((TagSeries) s).Pen == ((TrendObject) Trend).ScalePen);
                    scaleSeries?.UpdateAxisMaxMin();
                    foreach (var s in this.Series.Where(s => s.IsVisible))
                    {
                        if(s==scaleSeries)continue;
                        s.UpdateAxisMaxMin();
                    }
                }
                else
        ...
        public bool IsOnMouseWheel
        {
            set
            {
                _isOnMouseWheel = value;
                OnMouseWheel?.Invoke(this,EventArgs.Empty);
            }
            get { return _isOnMouseWheel; }
        }

        public event EventHandler OnMouseWheel;
        private bool _isOnMouseWheel;
        private Axis _defaultXAxis;
        private double _valueBarX;
        private double _valueBarDeltaX;
        private Axis _defaultYAxis;
        public bool ShowTitle { set; get; } = true;
        public bool DisplayPenIcon { set; get; } = false;
        public ScaleOption ScaleOption => Trend?.ScaleOption ?? ScaleOption.Independent;
        public bool DisplayX { set; get; } = true;
        public bool DisplayY { set; get; } = true;
        //properties for isolated trend
        public bool InIsolated { set; get; } = false;
        public double AxisYMaxLeft { set; get; } = 0;
        public Visibility Visibility { set; get; } = Visibility.Visible;
        public bool IsTmpVisibility { set; get; }
        public ObservableCollection<PlotModel> ParentCollection { set; get; }
        public string Name { set; get; }
        public bool ShowExtremity { set; get; } = false;

        public bool IsShowValueBar { set; get; } = true;

        public bool IsShowDelta { set; get; }
        public bool IsStruct { set; get; }

        public double ValueBarX
        {
            set
            {
                _valueBarX = value; 
                GetValueBarValues(true);
                if (IsShowValueBar&&!(Trend?.IsScrolling??true))
                    PlotView?.InvalidatePlot(false);
            }
            get { return _valueBarX; }
        }

        public double ValueBarDeltaX
        {
            set
            {
                _valueBarDeltaX = value;
                GetValueBarValues(false);
                if (IsShowValueBar && !(Trend?.IsScrolling ?? true))
                    PlotView?.InvalidatePlot(false);
            }
            get { return _valueBarDeltaX; }
        }

        

        internal List<Tuple<double,OxyColor,IPen>> Values { get; }=new List<Tuple<double, OxyColor, IPen>>();

        internal List<Tuple<double, OxyColor, IPen>> DeltaValues { get; }=new List<Tuple<double, OxyColor, IPen>>();

        public void ClearValueBar()
        {
            Values.Clear();
            DeltaValues.Clear();
        }
        
        private void GetValueBarValues(bool flag)
        {
            if (flag)
                Values.Clear();
            else
                DeltaValues.Clear();
            //var axisY = Axes.FirstOrDefault(a => !a.IsHorizontal());
            foreach (var series1 in Series)
            {
                //if(!series1.IsVisible)continue;
                var series = series1 as TagSeries;
                bool result;
                var p=series.GetY(flag?ValueBarX:ValueBarDeltaX,out result);
                if (!result) return;
                //var v = axisY.FormatValue(p.y);
                if (flag)
                {
                    _valueBarX = p.x;
                    Values.Add(new Tuple<double, OxyColor, IPen>(p.y, series.ActualColor,series.Pen));
                }
                else
                {
                    _valueBarDeltaX = p.x;
                    DeltaValues.Add(new Tuple<double, OxyColor, IPen>(p.y, series.ActualColor, series.Pen));
                }
                
            }
        }

        public bool IsBeginning { set; get; } = true;
    }
}
