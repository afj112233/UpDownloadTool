...
namespace OxyPlot
{
    ...
    public partial class PlotModel
    {
            ...
            lock (this.SyncRoot)
            {
                try
                {
                    ...
                    if (Trend != null)
                    {
                        if (!Series.Any())
                        {
                            var axis = Axes.FirstOrDefault(a => !a.IsHorizontal());
                            if (axis != null)
                            {
                    ...
                    //var axesYs = Axes.Where(a => !a.IsHorizontal()&&a.IsAxisVisible);
                    //var enumerable = axesYs as Axis[] ?? axesYs.ToArray();
                    //if (enumerable.Any())
                    //{
                    //    int count = 0;
                    ...
                    this.RenderAxes(rc, AxisLayer.BelowSeries, 1);
                    int offset = 0;
                    rc.Clean(ref offset,true);
                    this.RenderAxes(rc, AxisLayer.BelowSeries, 2);
                    rc.Clean(ref offset, false);

                    if (IsShowValueBar)
                    {
                        var axisX = Axes.FirstOrDefault(a => a.IsHorizontal()&&a.IsAxisVisible);

                        axisX?.RenderValueBarText(rc);
                    }
        ...
        private void SeriesGetData()
        {
            if (Trend == null) return;
            if (!Series.Any()) return;
            if (InIsolated && ScaleOption == ScaleOption.Same && !ShowTitle) return;
            var tagSeries = Series[0] as TagSeries;
            if (Trend.ChartStyle != ChartStyle.XY&&Trend.IsScrolling&&(tagSeries?.BufferPoints.Any() ?? false))
            {
                var lastX = tagSeries.BufferPoints.Last().x;
                if (double.IsNaN(lastX))
                    lastX = 0;
                if (Math.Abs(lastX) > Double.Epsilon && Math.Abs(tagSeries.EndX) > Double.Epsilon && Math.Abs(tagSeries.EndX - lastX) < double.Epsilon)
                {
                    Parallel.For(0, Series.Count, i =>
                    {
                        var series = Series[i] as TagSeries;
                        if (series != null)
                        {
                            while (!series.BufferPoints.IsEmpty)
                            {
                                DataPoint data;
                                var result = series.BufferPoints.TryDequeue(out data);
                            }
                        }
                    });
                    return;
            ...
            var spanX = Trend.TimeSpan.TotalSeconds;
            Parallel.For(0, Series.Count, i =>
            {
                var series = Series[i] as TagSeries;   
                if (series != null)
                {
                    ...
                    if (series.BufferPoints.IsEmpty)return;
                    double startX = -1;
                    bool isAbort = false;
                    double lastX = -1;
                    while (!series.BufferPoints.IsEmpty)
                    {
                        ...
                        if (Trend.ChartStyle == ChartStyle.Standard)
                        {
                            if (Math.Abs(startX - (-1)) < Double.Epsilon)
                            {
                                startX = data.X;
                                continue;
                            }
                    ...
                    if (Trend.ChartStyle==ChartStyle.Standard&&series.Points.Any()) 
                    {
                        series.EndX = series.Points.AsParallel().Last().X;
                    }
                }
            });
        }
        ...
        private static void EnsureMarginIsBigEnough(ref OxyThickness currentMargin, double minBorderSize, AxisPosition borderPosition,bool displayX)
        {
            switch (borderPosition)
            {
                case AxisPosition.Bottom:
                    currentMargin = new OxyThickness(currentMargin.Left, currentMargin.Top, currentMargin.Right, displayX?Math.Max(currentMargin.Bottom, minBorderSize):0);
                    break;
        ...
        private bool AdjustPlotMargins(IRenderContext rc)
        {
            ...
            for (var position = AxisPosition.Left; position <= AxisPosition.Bottom; position++)
            {
                ...
                EnsureMarginIsBigEnough(ref currentMargin, requiredSize, position,DisplayX);
            }
        ...
        /// <param name="rc">The render context.</param>
        /// <param name="layer">The layer.</param>
        /// <param name="pass">1.only render axesX;2.only render axesY;3.render all</param>
        private void RenderAxes(IRenderContext rc, AxisLayer layer, int pass = 3)
        {
            // render pass 0
            foreach (var a in this.Axes.Where(a => a.IsAxisVisible && a.Layer == layer))
            {
                if (pass == 3 || pass == 1 && a.IsHorizontal() || (pass == 2 && !a.IsHorizontal()))
                {
                    rc.SetToolTip(a.ToolTip);
                    a.Render(rc, 0);
                }
        ...
        private void UpdatePlotArea(IRenderContext rc)
        {
            ...
            if (InIsolated)
            {
                var margin=new OxyThickness(AxisYMaxLeft,ActualPlotMargins.Top,ActualPlotMargins.Right,ActualPlotMargins.Bottom);
                plotArea = plotArea.Deflate(margin);
            }
            else
            {
            ...
            //if (!DisplayY)
            //    plotArea = new OxyRect(0, plotArea.Top, plotArea.Width, plotArea.Height);
            if (!ShowTitle)
                plotArea = new OxyRect(plotArea.Left, 0, plotArea.Width, plotArea.Height);
            //FindVisualParentOfType<items>()
