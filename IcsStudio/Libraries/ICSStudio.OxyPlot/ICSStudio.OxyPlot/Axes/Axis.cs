...
namespace OxyPlot.Axes
{
    ...
    public abstract class Axis : PlotElement
    {
        ...
        protected Axis()
        {
            ...
            this.MinimumPadding = 10e-11;
            this.MaximumPadding = 10e-11;
            this.MinimumRange = 0;
        ...
        protected internal override OxyColor ActualTextColor
        {
            get
            {
                if (PlotModel?.Trend != null&&!IsHorizontal())
                {
        ...
        {
            get { return _isAxisVisible; }
            set
            {
                if (_isAxisVisible && !value)
                {

                }
                _isAxisVisible = value; 
            }
        }
        ...
        /// <summary>
        /// Gets or sets the layer of the axis. The default value is <see cref="OxyPlot.Axes.AxisLayer.BelowSeries"/>.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the 'padding' fraction of the maximum value. The default value is <c>10e-10</c>.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the 'padding' fraction of the minimum value. The default value is <c>10e-10</c>.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the position of the axis. The default value is <see cref="OxyPlot.Axes.AxisPosition.Left"/>.
        /// </summary>
        ...
        /// <remarks>Examples: The <see cref="OxyPlot.Axes.DateTimeAxis" /> returns the <see cref="DateTime" /> and <see cref="OxyPlot.Axes.CategoryAxis" /> returns category strings.</remarks>
...
#pragma warning restore 1718
        ...
        public bool IsZoomValid(double x1,double x2)
        {
#pragma warning disable 1718
            // ReSharper disable EqualExpressionComparison
            // ReSharper disable CompareOfFloatsByEqualityOperator
...
#pragma warning restore 1718
        ...
        public void RenderValueBarText(IRenderContext rc)
        {
            var r = new HorizontalAndVerticalAxisRenderer(rc, this.PlotModel);
            r.RenderValueBar(this, 1);
            r.RenderValueBar(this, 2);
        }
        ...
        public virtual void ZoomAt(double factor, double x)
        {
            ...
            this.scale *= factor;
            if (!this.IsZoomValid(dx1, dx0))
            {
                MessageBox.Show("Unable to zoom any further more","ICS Studio",MessageBoxButtons.OK,MessageBoxIcon.Asterisk);
                return;
            }
        ...
        {
            if (!PlotModel.IsBeginning && double.IsNaN(_keepMax))
            {
                _keepMax = ActualMaximum;
                _keepMin = ActualMinimum;
            }
        ...
        internal virtual void UpdateActualMaxMin()
        {
            ...
            if (PlotModel.Trend == null)
            {
                ...
                {
                    if (this.ActualMaximum > 0)
                    {
                        this.ActualMinimum = this.ActualMinimum / 2;
                    }
                    else
                    {
                        this.ActualMaximum = this.ActualMinimum / 2;
                    }
                }
            }
        ...
        internal virtual void UpdateTransform(OxyRect bounds)
        {
            ...
            var series = PlotModel.Series.FirstOrDefault(s => (s as LineSeries)?.LeftAxes == this) as LineSeries;
            if (PlotModel.ScaleOption == ScaleOption.Independent && series != null)
            {
                ResetDataMaxMin();
                series.UpdateMaxMin();
        ...
        /// See <see cref="OxyPlot.Axes.LogarithmicAxis" /> for examples on how to implement this.</remarks>
        ...
        protected virtual IList<double> CreateTickValues(double from, double to, double step, int maxTicks = 1000)
        {
            if (PlotModel.Trend != null)
            {
                return AxisUtilities.CreateTickValues(from, to, step, maxTicks,false);
            }
        ...
        protected virtual void CoerceActualMaxMin()
        {
            ...
            if (double.IsNaN(this.ActualMaximum) || double.IsInfinity(this.ActualMaximum))
            {
                needKeep = false;
                if (PlotModel?.Trend != null)
                    this.ActualMaximum = 1;
                else
                    this.ActualMaximum = 100;
                if (PlotModel?.Trend != null&&PlotModel.Trend.ChartStyle==ChartStyle.Standard)
                {
                    if (this is TimeSpanAxis)
                        this.ActualMaximum = PlotModel.Trend.TimeSpan.TotalSeconds;
                    if (!PlotModel.Series.Any() && this is LinearAxis)
                        this.ActualMaximum = 1;
                }
            }
            ...
            if (!(this is TimeSpanAxis))
            {
                if (PlotModel?.Trend != null && PlotModel.Trend.ValueOption == ValueOptionType.Automatic)
                {
                    if (PlotModel.InIsolated)
                    {
                        if (PlotModel.ScaleOption != ScaleOption.UsingPen&&needKeep)
                        {
                            if (_keepMax > ActualMaximum)
                            {
                                var tmp = ActualMaximum;
                                ActualMaximum = _keepMax;
                                _keepMax = tmp;
                                //if(IsNear(tmp,ActualMinimum))
                                ////if (Math.Abs((decimal)tmp - (decimal)ActualMinimum) > 2*(decimal)GetPadding(ActualMaximum))
                                //    _keepMax = tmp;
                                //else
                                //    _keepMin = ActualMinimum;
                            }
                            ...
                            if (_keepMin < ActualMinimum)
                            {
                                var tmp = ActualMinimum;
                                ActualMinimum = _keepMin;
                                _keepMin = tmp;
                                //if(IsNear(tmp,ActualMaximum))
                                ////if (Math.Abs((decimal)tmp - (decimal)ActualMaximum) > 2 * (decimal)GetPadding(ActualMinimum))
                                //    _keepMin = tmp;
                                //else
                                //    _keepMax = ActualMaximum;
                            }
        ...
        //private bool IsNear(double val1,double val2)
        //{
        //    try
        //    {
        //        var d1 = (decimal) val1;
        //        var d2 = (decimal) val2;
        //        return Math.Abs(d1 - d2) > 2 * (decimal) GetPadding(val2);
        //    }
        //    catch (Exception)
        //    {
        //        return Math.Abs(val1 - val2) > 2 * GetPadding(val2);
        //    }
        //}

        private double _keepMax=Double.NaN;
        private double _keepMin=Double.NaN;

        public void ResetKeepMaxMin()
        {
            _keepMax = Double.NaN;
            _keepMin = Double.NaN;
        }
        ...
        protected virtual string FormatValueOverride(double x)
        {
            ...
            var place = PlotModel.Trend.YScaleDecimalPlaces;
            ...
            if (Math.Abs(x) < 1e-8 && x != 0)
            {
                return x.ToString("0.000000e0");
            }

            string format = string.Concat("{0:", this.StringFormat ?? this.ActualStringFormat ?? string.Empty, "}");

            if (x > 999999 || x < -999999)
                format = string.Concat("{0:", GetDefaultStringFormat(), "}");
        ...
        protected virtual double CalculateActualMaximum()
        {
            ...
            if (range < double.Epsilon)
            {
                ...
                //if (FormatValue(actualMaximum).Contains("e"))
                //{
                //    actualMaximum += actualMaximum/10e7*5;
                //}
                //else
                //{
                //    actualMaximum += 10e-10;
                //}
                var padding = GetPadding(actualMaximum);
                actualMaximum += padding<0?-padding:padding;
            }
        ...
        protected virtual double CalculateActualMinimum()
        {
            ...
            if (range < double.Epsilon)
            {
                ...
                var padding = GetPadding(actualMinimum);
                actualMinimum -= padding<0?-padding:padding;
            }
        ...
        private double GetPadding(double val)
        {
            if(val == 0)
                return 10e-10;
            var place = PlotModel.Trend.YScaleDecimalPlaces;
            var correct = Math.Pow(10, place * -1);
            //if (FormatValue(val).Contains("e"))
                //return val / 10e8 * 5;
            return correct*val;
            //return 10e-10;
        }
        ...
        public int MajorGridLines { set; get; } = -1;
        public int MinorGridLines { set; get; } = -1;

        public bool ScaleAsPercentage { set; get; } = false;
    }
}