...
namespace OxyPlot.Series
{
    ...
    public class LineSeries : DataPointSeries
    {
        ...
        public object syncRoot {protected set; get; } = new object();
        /// <summary>
        /// The output buffer.
        /// </summary>
        ...
        public LineSeries()
        {
            LeftAxes = new LinearAxis() { Position = AxisPosition.Left ,IsAxisVisible = false};
        ...
        public OxyColor Color
        {
            ...
            {
                _color = value;
                LeftAxes.UserDefinedColor = value;
            }
        }
        ...
        public DataPoint GetY(double x, out bool result)
        {
            lock (syncRoot)
            {
                result = true;
                ...
                var sample = (PlotModel.Trend?.SamplePeriod ?? double.Epsilon) / 1000;
                var p = Points.AsParallel().FirstOrDefault(s => Math.Abs(s.x - x) <= sample);
                if (Math.Abs(p.x - x) <= sample)
                    return p;
                var front = Points.AsParallel().LastOrDefault(s => s.x < x);
                var back = Points.AsParallel().FirstOrDefault(s => s.x > x);
                var sp = Transform(x, 0);
                double dx1 = Math.Abs(front.x - sp.x), dx2 = Math.Abs(back.x - sp.X);
                if (dx1 < dx2)
                    return front;
                else return back;
            }
