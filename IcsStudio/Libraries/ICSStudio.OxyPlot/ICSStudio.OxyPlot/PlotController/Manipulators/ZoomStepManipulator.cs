...
namespace OxyPlot
{
    ...
    public class ZoomStepManipulator : MouseManipulator
    {
        ...
        public override void Started(OxyMouseEventArgs e)
        {
            ...
            if (PlotView.ActualModel.Trend != null)
            {
                foreach (var ax in PlotView.ActualModel.Axes)
                {
                    if (ax.IsHorizontal())
                    {
                        ax.ZoomAt(scale,current.x);
                    }
                    else
                    {
                        ax.ZoomAt(scale,current.y);
                    }
                }
            }
            else
            ...
            if (PlotView.ActualModel.Trend != null)
            {
                PlotView.ActualModel.Trend.IsUpdateScale = true;
            }
            e.Handled = true;
        }
    }
