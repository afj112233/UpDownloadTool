...
{
    public class Int64ValidationRule : IntegerValidationRule
    {
        public override ValidationResult Validate(object value, CultureInfo cultureInfo)
        {
            ...
            if (Wrapper.DisplayStyle == DisplayStyle.Ascii)
            {
                try
                {
                    ...
                    if (bytes.Count > 0 && bytes.Count <= 8)
                ...
                catch (Exception ex)
                {
                    ...
                    System.Windows.MessageBox.Show(errorContent);
                    return new ValidationResult(false, errorContent);
                }
            }
            ...
            try
            {
                if (input.StartsWith("2#"))
                {
                    ...
                    var unused = Convert.ToInt64(input, 2);
                }
                else if (input.StartsWith("8#"))
                {
                    ...
                    var unused = Convert.ToInt64(input, 8);
                }
                else if (input.StartsWith("16#"))
                {
                    ...
                    var unused = Convert.ToInt64(input, 16);
                }

                else if (Wrapper.DisplayStyle == DisplayStyle.Binary)
                {
                    var unusedBinary = Convert.ToInt64(input, 2);
                }

                else if (Wrapper.DisplayStyle == DisplayStyle.Octal)
                {
                    var unusedOctal = Convert.ToInt64(input, 8);
                }

                else if (Wrapper.DisplayStyle == DisplayStyle.Hex)
                {
                    var unusedHex = Convert.ToInt64(input, 16);
                }

                else
                {
                    var unused = long.Parse(input);
                }
            }
            catch (Exception)
            {
