...
{
    public class SByteValidationRule : IntegerValidationRule
    {
        public override ValidationResult Validate(object value, CultureInfo cultureInfo)
        {
            ...
            try
            {
                ...
                {
                    if (Wrapper.DisplayStyle == DisplayStyle.Binary)
                    {
                        var unusedBinary = Convert.ToByte(input, 2);
                    }

                    if (Wrapper.DisplayStyle == DisplayStyle.Octal)
                    {
                        var unusedOctal = Convert.ToByte(input, 8);
                    }

                    if (Wrapper.DisplayStyle == DisplayStyle.Hex)
                    {
                        var unusedHex = Convert.ToByte(input, 16);
                    }

                    var unused = sbyte.Parse(input);
                }
            }
            catch (Exception)
            {
